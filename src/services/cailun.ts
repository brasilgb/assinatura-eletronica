...
const cailun = axios.create({
  withCredentials: true,
});

cailun.interceptors.request.use(async (request) => {
  // request.baseURL = process.env.EXPO_PUBLIC_API_URL;
  // BASE_URL = `${process.env.EXPO_PUBLIC_API_URL}`;
  request.baseURL =
    "http://172.16.1.215:9090/servicesgruposolar/servlet/isCobol/";
...
cailun.interceptors.response.use(
  (response) => response,
  async (_error) => {
    ...
    if (requestCustom.method === "POST" || requestCustom.method === "post") {
      originalResponse = await cailun.post(`${requestCustom.url}`, data);
    } else {
      originalResponse = await cailun.get(`${requestCustom.url}`);
    }
