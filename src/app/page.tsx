    ...
    return (
      <div className="absolute top-0 right-0 bottom-0 left-0 bg-gray-800 bg-opacity-10 flex items-center justify-center">
        <div className="w-1/3 bg-white rounded-md shadow-md p-1">
          <div className="p-2 text-solar-gray-light bg-solar-blue-primary rounded-t">
            Copiado
          </div>
          <div className="pt-4 px-2 bg-gray-100 text-lg text-center text-gray-600">
            Link copiado para a área de transferência
          </div>
          <div className="px-2 py-2 bg-gray-100 text-lg flex items justify-center text-gray-600">
            <div className="text-xs bg-gray-300 rounded py-0.5 px-1">
              {linkCopied}
            </div>
          </div>
    ...
    <>
      ...
      <main className="container mx-auto">
        <div className="flex items-center justify-start py-4">
          <h1 className="text-xl text-solar-blue-secundary font-bold mr-2 uppercase">
            Assinatura eletrônica
          </h1>
          <span className="text-gray-500">-</span>
          <h1 className="text-lg text-gray-700 ml-2">
            acompanhe os status das assinaturas
          </h1>
        </div>
        <SubBarTop />
        <div className="mt-6 bg-gray-50 shadow rounded-md p-2">
          <table className="table-auto w-full text-left text-gray-600 bg-solar-blue-secundary rounded-t-md">
            ...
            {assignDocs?.map((doc: any, idx: number) => (
              <tr
                key={idx}
                className={`${idx < assignDocs.length - 1 && "border-b border-gray-20"} ${idx % 2 > 0 ? "bg-gray-50" : "bg-gray-100"} hover:bg-pink-50`}
              >
                ...
                <td className="p-1">
                  {moment(doc.creationDate).format("DD/MM/YYYY")}
                </td>
